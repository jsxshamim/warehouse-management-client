...
const RecentInventories = () => {
    ...
    const [inventories, , loading] = useGetData("https://easystock-server.herokuapp.com/inventories");
    ...
    return (
        <section className="container mx-auto py-20 shadow-lg">
            ...
            <div className="inventories grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 mb-14 px-5 relative">
                {loading ? <Spinner /> : inventories.slice(0, 6).map((inventory) => <RecentInventory key={inventory._id} inventory={inventory} />)}
            </div>
